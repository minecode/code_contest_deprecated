...
const Drop: React.FC = () => {
    ...
    setFileName(acceptedFile.map((file: any) => {
      ...
      reader.addEventListener('load', async () => {
        if (reader) {
          ...
          try {
            const fileAlreadyExist = await apiCodeContest.get(`/contents/contests/${challengeName?.split(' ').join('_')}/${auth.user.id}/resolution.py`, config)
  ...
  const submitFile = async (bodyRequest: BodyRequest) => {
    setSolution('')
    setInProgress(true)
    await apiCodeContest.put(`/contents/contests/${challengeName?.split(' ').join('_')}/${auth.user.id}/resolution.py`, bodyRequest, config)
  }
